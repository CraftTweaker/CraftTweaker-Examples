...
events.register<EntityPlaceBlockEvent>((event) => {
    //event.placedBlock is a blockstate, with properties, rather than the Block.
    if (event.placedBlock.block != <block:minecraft:campfire>) return;
    val myLevel = event.level as Level;
    if (myLevel.isClientSide) {
        return; //We can only execute commands (and do most functional work) in the logical side.
    }
    val server = (myLevel as ServerLevel).server;
    //Execute time set day without the command set to silent.
    server.executeCommand("time set day", false);
});

/*
* Some important bits to know with events:

* - You shouldn't ever register an event listener within another event listener.
* - All functional work should happen on the logical server side (Remember, singleplayer still has a server side and a client side)
* - Events might fire more than one time due to sidedness, mods, or other reasons. 
    In situations like this one, try to println event parameters to see what is changing.
*/